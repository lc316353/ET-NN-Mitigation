# -*- coding: utf-8 -*-
"""
Created on Sun Sep 10 19:37:01 2023
...
L=8000                         #Length of simulation box
Nx=150
Nt=500
...
NoE=15                          #Number of events per run
NoS=6                          #Number of seismometers


x=np.linspace(-L,L-dx,grid_size)
y=np.linspace(-L,L-dx,grid_size).T
xy=np.meshgrid(x,x)
x2d=xy[1]
y2d=xy[0]
...
def calc_force(drho,R=0):
    ...
    for ix in range(grid_size):
        for iy in range(grid_size):
            ...
            if r<=R:
                continue
            F+=G*M*dx**2*np.cos(np.arctan2(y[iy],x[ix]))*drho[ix,iy]/r**2
    return F
...
seismometer_data=[]                 #x- and y-displacement for each seismometer

#Seismometer
seismometer_position_index=[]
seismometer_positions=np.zeros((NoS,2))

for s in range(NoS):
    seismometer_position_index.append([rd.randint(0,grid_size-1),rd.randint(0,grid_size-1)])
    seismometer_data.append([[],[]])
seismometer_position_index=np.array(seismometer_position_index)
seismometer_positions[:,0],seismometer_positions[:,1]=x[seismometer_position_index[:,0]],y[seismometer_position_index[:,1]]
...
for n in range(NoE):
    ...
    angles.append(rd.random()*2*pi)#rd.randint(0,359)*pi/180)
    #phis.append(rd.random()*2*pi)#rd.randint(0,359)*pi/180)
    ...
    zs.append(np.cos(angles[n])*x2d+np.sin(angles[n])*y2d)
...
wave_params=[angles,x0s,t0s,fs,sigmafs,As]
wave_param_texts=["angle","x0","t0","f","sigma_f","A_wave"]
...
for t in time:
    #print(t)
    density_fluctuations=np.zeros((grid_size,grid_size))

    #print("t1",systime.time())
    for n in range(NoE):
        density_fluctuations+=gauss_wave_package_analytic(zs[n], t, x0s[n], t0s[n], fs[n], sigmafs[n], As[n],phases[n])
    ...
    for s in range(NoS):
        ...
        ydisplacement=0
        for n in range(NoE):
            distanceFromWave=seismometer_positions[s][0]*np.cos(angles[n])+seismometer_positions[s][1]*np.sin(angles[n])-x0s[n]-c_s*(t-t0s[n])
            ...
            xdisplacement+=np.cos(angles[n])*absoluteDisplacement
            ydisplacement+=np.sin(angles[n])*absoluteDisplacement
...
run_info=np.loadtxt("2Ddata/run_info.txt")
run_id=0#int(run_info[-1][0])+1 #TODO: Make this all a function
print("run ID: ", run_id)
np.savetxt("2Ddata/eventdata"+str(run_id)+".txt",[angles,x0s,t0s,fs,sigmafs,As,phases])
np.savetxt("2Ddata/seismometerPositions"+str(run_id)+".txt",seismometer_positions,fmt="%.4e")
np.savetxt("2Ddata/forceOutput"+str(run_id)+".txt",force)
np.savetxt("2Ddata/seismometerInput"+str(run_id)+".txt",[seismometer_data[int(i/NoS)][i%2] for i in range(2*NoS)])
run_info=list(run_info)
run_info.append(np.array([run_id,0,systime.time()-start_time]))
np.savetxt("2Ddata/run_info.txt",run_info,fmt="%.3e")
...
im=plt.imshow(density_fluctuation_array[timestep][:,::-1].T,extent=[-L,L,-L,L],label=r"$\delta\rho$")
...
def animate():
    def update(i):
        ...
        im.set_array(density_fluctuation_array[i][:,::-1].T)
    ...
    anima=ani.FuncAnimation(fig=fig, func=update, frames=int(tmax/dt)+1,interval=5)
...
lines=[[],[]]
plotspace=2
titles=[r"seismometer $x$-displacement",r"seismometer $y$-displacement"]
max_dim=2
for s in range(min(NoS,6)):
    for dim in range(max_dim):
        axarray[dim].append(plt.subplot(2*NoS+plotspace,6,(NoS*6+(int(6/max_dim)*dim+1)+(s+plotspace)*6,NoS*6+(int(6/max_dim)*dim+3)+(s+plotspace)*6)))
        color=[0,0,0]
