...
if dim==2 and "sphere" in mode.lower():
    ...
    lower_bound = [-5000.,-5000.,-300.]
    upper_bound = [5000.,5000.,300.]
...
def foo(N=10, freq=1, SNR=15, p=0.2, mirror="all", ID=0, savename="Results", worker=1, animate=False):
        ...
        if "particleswarm" in optimizationMethod.lower():
            ...
            swarm_size = 400 #For 4000 choose 4000 MB memory
        ...
        if animate:
            ...
            if "particleswarm" in optimizationMethod.lower():
                ...
                for j in range(dim*min(N,3)): 
                    plt.plot(pos_hist[:,0,j],label="no. {}, {}-pos".format(j//dim,["x","y","z"][j%dim]))
                plt.legend()
...
if __name__ == '__main__':
        ...
        savename = mode+"Results" #str(argv[3])
